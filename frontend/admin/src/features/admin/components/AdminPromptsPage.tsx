       ...
       <div className="bg-dark-card border border-border rounded-xl overflow-hidden flex-1 flex flex-col">
          ...
          {totalPages > 1 && (
             <div className="border-t border-border p-1 md:p-4 bg-zinc-900/50 flex justify-center items-center gap-0.5 md:gap-2 mt-auto">
                <Button variant="ghost" size="icon" disabled={page === 1} onClick={() => { handlePageChange(page - 1); }} className="h-8 w-8 md:h-10 md:w-10 disabled:opacity-50">
                   <ArrowLeft size={14} className="md:w-4 md:h-4" />
                </Button>
                <span className="text-[10px] md:text-sm text-text-secondary flex-shrink-0 px-1 md:px-2">
                   {page}/{totalPages}
                </span>
                <Button variant="ghost" size="icon" disabled={page === totalPages} onClick={() => { handlePageChange(page + 1); }} className="h-8 w-8 md:h-10 md:w-10 disabled:opacity-50">
