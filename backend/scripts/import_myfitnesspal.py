        ...
        for row_num, row in enumerate(reader, start=2):  # Start at 2 (header is 1)
            try:
                ...
                try:
                    ...
                    print(f"  ⚠️  Linha {row_num}: Formato de data inválido '{date_str}', pulando...")
                    continue
...
def import_nutrition(
    ...
    for date_str in sorted_dates:
        ...
        print(f"{date_str:<12} {int(log.calories):>6} {log.protein_grams:>6.1f} {log.carbs_grams:>6.1f} {log.fat_grams:>6.1f} {status}")
...
automaticamente todas as refeições de cada dia em um único registro diário.
    ...
    parser.add_argument(
        "user_email",
        help="Email do usuário para associar os dados"
    )
    parser.add_argument(
        "csv_path",
        help="Caminho para o arquivo CSV exportado do MyFitnessPal"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Preview dos dados sem salvar no banco"
    )
