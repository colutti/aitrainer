...
class WorkoutStats(BaseModel):
    ...
    current_streak_weeks: int = Field(
        ..., description="Consecutive weeks with >= 3 workouts"
    )
    weekly_frequency: list[bool] = Field(
        ..., description="List of 7 bools indicating workout days (Sun-Sat or Mon-Sun)"
    )
    weekly_volume: list[VolumeStat] = Field(
        ..., description="Volume per category for the current week"
    )
