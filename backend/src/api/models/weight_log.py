...
class WeightLogInput(BaseModel):
    ...
    muscle_mass_pct: float | None = Field(None, ge=0, le=100, description="Muscle mass percentage")
    bone_mass_kg: float | None = Field(None, ge=0, lt=20, description="Bone mass in kg")
    body_water_pct: float | None = Field(None, ge=0, le=100, description="Body water percentage")
    visceral_fat: float | None = Field(None, ge=0, le=50, description="Visceral fat level")
