...
class TrainerRepository(BaseRepository):
    ...
    def save_profile(self, trainer_profile: TrainerProfile) -> None:
        ...
        if result.upserted_id:
            self.logger.info("New trainer profile created for user: %s", trainer_profile.user_email)
        elif result.modified_count > 0:
            self.logger.info("Trainer profile updated for user: %s", trainer_profile.user_email)
        else:
            self.logger.debug("Trainer profile for user %s already up-to-date or no changes.", trainer_profile.user_email)
