...
class WeightRepository(BaseRepository):
    ...
    def save_log(self, log: WeightLog) -> tuple[str, bool]:
        ...
        if is_new:
            ...
            self.logger.info("Created new weight log for %s on %s", log.user_email, log.date)
        else:
            existing = self.collection.find_one(
                {"user_email": log.user_email, "date": log_datetime}
            )
            doc_id = str(existing["_id"]) if existing else ""
            self.logger.info("Updated existing weight log for %s on %s", log.user_email, log.date)
    ...
    def delete_log(self, user_email: str, log_date: date) -> bool:
        ...
        self.logger.warning("Attempted to delete non-existent weight log for %s on %s", user_email, log_date)
        return False
