...
class AdaptiveTDEEService:
    ...
    def _compute_tdee_observations(
        ...
        """
        Calculates daily TDEE observations from weight trends and nutrition data.

        Formula (MacroFactor):
        obs_TDEE_t = calories_t - (trend_weight_t - trend_weight_{t-1}) × energy_per_kg

        Args:
            daily_trend: dict of {date: trend_weight} (output from Task 2)
            nutrition_by_date: dict of {date: calories}
            body_fat_pct: user's body fat percentage (can be None for default 7700)
            trend_slope: daily weight change for energy_per_kg estimation

        Returns:
            List of daily TDEE observations in chronological order.
        """
        if not daily_trend or not nutrition_by_date:
            return []
        ...
        energy_per_kg = self._estimate_energy_per_kg(body_fat_pct, trend_slope)
        observations = []
        ...
        for i in range(1, len(sorted_dates)):
            ...
            # Skip if no nutrition data for this day
            if curr_date not in nutrition_by_date:
                continue

            curr_trend = daily_trend[curr_date]
            prev_trend = daily_trend[prev_date]
            trend_change = curr_trend - prev_trend
            calories = nutrition_by_date[curr_date]

            obs_tdee = calories - (trend_change * energy_per_kg)
            observations.append(obs_tdee)
    ...
    def _compute_tdee_from_observations(
        ...
        """
        Calculates TDEE using EMA of observations with Bayesian prior.

        EMA formula:
        - Initialize: ema = formula_prior
        - For each obs: ema = alpha * obs + (1 - alpha) * ema
        - alpha = 2 / (span + 1)

        Args:
            observations: list of daily TDEE observations
            formula_prior: initial TDEE estimate (Mifflin-St Jeor, etc.)
            span: EMA window (default 21 days)

        Returns:
            Estimated TDEE as float.
        """
        if not observations:
            return formula_prior
