...
def test_create_invite(invite_repo, sample_invite):
    ...
    invite_repo.collection.insert_one.assert_called_once_with(sample_invite.model_dump())
...
def test_revoke_success(invite_repo, sample_invite):
    ...
    assert result is True
    invite_repo.collection.delete_one.assert_called_once_with({"token": sample_invite.token})
