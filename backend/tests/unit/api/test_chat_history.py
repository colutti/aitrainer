...
def test_chat_history_clean_text():
    ...
    assert msg.get_clean_text() == "Hello world! How are you?"
...
def test_chat_history_formatted_timestamp_valid():
    ...
    assert msg.get_formatted_timestamp() == "01/01/2024 10:00"
...
def test_chat_history_formatted_timestamp_invalid():
    ...
    assert msg.get_formatted_timestamp() == "invalid-date"
...
def test_chat_history_sender_label():
    ...
    assert "Aluno" in student_msg.get_sender_label()
    assert "Treinador" in trainer_msg.get_sender_label()
