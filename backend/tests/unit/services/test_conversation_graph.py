...
@pytest.mark.asyncio
async def test_training_specialist_does_not_receive_hevy_tools_for_generic_import():
    """Connected user + generic import word should NOT expose Hevy tools."""
    runner, brain = _runner_with_brain()
    captured = {}
    tool_names = [
        "save_workout", "get_workouts", "list_hevy_routines", "get_body_composition",
    ]
    tool_mocks = []
    for tool_name in tool_names:
        tool = MagicMock()
    ...
    brain.get_log_callback.return_value = None
    stub_profile = MagicMock()
    stub_profile.hevy_enabled = True
    stub_profile.hevy_api_key = "some-key"
    brain.database.get_user_profile.return_value = stub_profile
    ...
    state = GraphState(
        user_email="a@b.com",
        user_input_raw="quero importar meu historico",
        user_input_sanitized="quero importar meu historico",
        channel="app",
    )
    state.shared_context = {
        "input_data": {
...
@pytest.mark.asyncio
async def test_plan_specialist_missing_upsert_when_existing_plan_but_training_not_ready():
    """Even with an existing plan, upsert_plan must be stripped when
    training proposal is not ready — structural changes require fresh proposal.
    """
    runner, brain = _runner_with_brain()
    captured = {}
    ...
    state = GraphState(
        user_email="a@b.com",
        user_input_raw="ajusta o plano",
        user_input_sanitized="ajusta o plano",
        channel="app",
    )
    state.shared_context = {
        "input_data": {
            "user_locale": "pt-BR",
            "runtime_context_json": "{}",
            "plan_section": "existing_plan_active",
            "agenda_section": "",
            "metabolism_section": "",
        },
        "training_workspace": {
