...
class TestWorkoutEndpoints(unittest.TestCase):
    ...
    def test_list_workouts_success(self):
        ...
        mock_db.get_workouts_paginated.assert_called_with(
            user_email="test@test.com", page=1, page_size=10, workout_type=None
        )
    ...
    def test_list_workouts_with_filter(self):
        ...
        self.client.get(
            "/workout/list?workout_type=Pull", headers={"Authorization": "Bearer token"}
        )

        mock_db.get_workouts_paginated.assert_called_with(
            user_email="test@test.com", page=1, page_size=10, workout_type="Pull"
        )
        app.dependency_overrides = {}
    ...
    def test_list_workouts_error(self):
        ...
        response = self.client.get(
            "/workout/list", headers={"Authorization": "Bearer token"}
        )
