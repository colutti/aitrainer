...
def mock_user_profile():
    return UserProfile(
        ...
        hevy_api_key="stored_key", hevy_enabled=True
    )
...
def test_import_endpoint(mock_hevy_service, mock_brain):
    ...
    response = client.post("/integrations/hevy/import", json={"mode": "skip_duplicates"})
    assert response.status_code == 200
...
def test_missing_key_import(mock_hevy_service, mock_brain):
    ...
    no_key_profile = UserProfile(
        email="test@example.com",
        gender="Masculino", age=25, weight=70, height=175, goal="Get Strong", goal_type="gain",
        hevy_api_key=None
    )
