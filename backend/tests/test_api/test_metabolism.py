...
def test_get_metabolism_summary():
    ...
    weights = [WeightLog(user_email="test@example.com", date=start_date + timedelta(days=i), weight_kg=70.0) for i in range(21)]
    nutrition = [NutritionLog(user_email="test@example.com", date=start_date + timedelta(days=i), calories=2500, protein_grams=150, carbs_grams=250, fat_grams=80) for i in range(21)]
...
def test_get_metabolism_summary_insufficient():
    ...
    # Act
    response = client.get("/metabolism/summary", headers={"Authorization": "Bearer token"})
