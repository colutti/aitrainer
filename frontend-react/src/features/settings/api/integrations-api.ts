...
export const integrationsApi = {
  // Hevy
  getHevyStatus: async (): Promise<HevyStatus> => {
    return httpClient('/integrations/hevy/status');
  },
  
  saveHevyKey: async (apiKey: string): Promise<HevyStatus> => {
  ...
  syncHevy: async (): Promise<{ message: string; workouts: number }> => {
  ...
  // Telegram
  getTelegramStatus: async (): Promise<{ connected: boolean; username?: string }> => {
     return httpClient('/integrations/telegram/status');
  },
  
  generateTelegramCode: async (): Promise<{ code: string; url: string }> => {
