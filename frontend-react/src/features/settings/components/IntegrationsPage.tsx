...
import { Upload, Check, AlertCircle, RefreshCw, Smartphone, Download, Database, Send } from 'lucide-react';

import { Button } from '../../../shared/components/ui/Button';
import { Input } from '../../../shared/components/ui/Input';
import { useNotificationStore } from '../../../shared/hooks/useNotification';
import { integrationsApi } from '../api/integrations-api';
import { HevyStatus, ImportResult } from '../../../shared/types/integration';
...
export function IntegrationsPage() {
  ...
  async function handleSyncHevy() {
    ...
    try {
      ...
      notify.success(`Sincronização iniciada: ${res.message}. Workouts: ${res.workouts}`);
    } catch {
  ...
  async function handleUpload(file: File, type: 'mfp' | 'zepp') {
    ...
    try {
      ...
      notify.success(`Importação concluída! Criados: ${res.created}, Atualizados: ${res.updated}`);
      if (res.errors > 0) {
        notify.info(`${res.errors} erros encontrados durante a importação.`);
      }
    } catch (err) {
      console.error(err);
