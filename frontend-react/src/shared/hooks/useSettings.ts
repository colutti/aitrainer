...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  updateProfile: async (data: Partial<UserProfile>) => {
    ...
    try {
      const updated = await httpClient<UserProfile>('/user/profile', {
        method: 'POST',
  ...
  fetchTrainer: async () => {
    try {
      const trainer = await httpClient<TrainerProfile>('/trainer/profile');
  ...
  fetchAvailableTrainers: async () => {
    try {
      const trainers = await httpClient<TrainerCard[]>('/trainer/available');
  ...
  updateTrainer: async (trainerType: string) => {
    set({ isSaving: true });
    try {
      const updated = await httpClient<TrainerProfile>('/trainer/profile', {
        method: 'POST',
